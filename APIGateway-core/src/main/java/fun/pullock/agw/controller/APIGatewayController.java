package fun.pullock.agw.controller;
...
import static fun.pullock.agw.model.ErrorCode.PARAM_ERROR;
...
public class APIGatewayController {
    ...
    public Result<String> v1Service(
        ...
        try {
            ...
            params.put(Constants.SYS, sys);
            params.put(Constants.API_NAME, apiName);
            params.put(Constants.OUT_NAME, outName);
            params.put(Constants.NONCE, nonce);
            params.put(Constants.SIGNATURE, signature);
            params.put(Constants.PARAM, param);
            ...
            String xff = request.getHeader(Constants.X_FORWARDED_FOR);
            if (StringUtils.isNotEmpty(xff)) {
                LOGGER.info("request {}: {}", Constants.X_FORWARDED_FOR, xff);
